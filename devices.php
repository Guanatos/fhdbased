...
$myquery = "SELECT type_id, type, type_name FROM site_types WHERE type LIKE 3 ORDER BY type_name;";
$site_calls = $db->get_results($myquery);
//$db->debug();
$num = $db->num_rows;
echo "<p><a href='fhd_settings.php'>Back to Settings</a></p>";
echo "<h4>$num $title Devices</h4>";
...
//		echo "<td>$type_desc</td>\n";
		echo "<td align='center'><a href='mod_devices.php?type_id=$type_id&action=edit'><i class='glyphicon glyphicon-edit' title='edit'></i></a></td>\n";
       	$deletelink = "<a href='devices.php?type_id=$type_id&action=delete&nacl=$nacl' onclick=\"return confirm('Are you sure you want to delete?')\"><i class='glyphicon glyphicon-remove-circle' title='delete'></i></a>";
		echo "<td align='center'>$deletelink</td>\n";
		echo "</tr>\n";
		} // foreach
?>
<h5><i class="fa fa-plus"></i> <a href="dnr_add_devices.php">Add New</a></h5>
<?php } ?> 
</table>
<?php
